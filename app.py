...
BOSS_LINE = "😡 JEFE: <b>TU PUESTO PARECE QUE PRONTO QUEDARÁ VACANTE</b>"
TIME_LIMIT = 60  # 1 minuto por pregunta

state = {
    "level": 0,
    "level_start": None,  # momento en que inició el nivel actual
}
...
def show_screen(title, text, controls, show_timer_note=False):
    ...
    if show_timer_note:
        display(w.HTML(
            ...
            "<b>⏱️ Regla:</b> 60 segundos por pregunta. "
...
def timeout_screen(retry_level):
    ...
    show_screen(
        "⛔ TIEMPO AGOTADO",
        boss_warning("Se acabó el minuto. En auditoría, el tiempo también es evidencia."),
...
def go(level):
    ...
    if level == 0:
        ...
        btn = w.Button(description="INICIAR AUDITORÍA")
        btn.on_click(lambda _: go(1))
        show_screen(
            "AUDITORÍA FORENSE – NIC 16",
            """
            Son las 22:45. Usted es el auditor externo de <b>INDUSTRIAL ANDINA S.A.</b><br><br>
            Alerta anónima: <i>"Están manipulando activos fijos para inflar utilidades."</i><br><br>
            Regla: <b>1 minuto por pregunta</b>. Si falla… el jefe se entera.
            """,
            [btn],
            show_timer_note=False
        )
    ...
    if level == 1:
        ...
        show_screen(
            ...
            """
            La empresa compró una máquina por <b>$80.000</b> para usarla en producción durante 10 años.<br><br>
            ¿Cómo debe reconocerse según NIC 16?
            """,
            buttons + [msg],
    ...
    if level == 2:
        ...
        def verify(_):
            ...
            try:
                ...
                msg.value = boss_warning("Ingrese un número válido (solo números).")

        btn.on_click(verify)

        show_screen(
            "Nivel 2: Medición inicial",
            """
            La empresa incurrió en:<br>
            • Precio: 80.000<br>
            • Transporte: 5.000<br>
            • Instalación: 3.000<br><br>
            ¿Cuál es el <b>costo inicial</b> del activo?
            """,
            [inp, btn, msg],
            show_timer_note=True
        )
    ...
    if level == 3:
        ...
        def check(ans):
            ...
            if ans == "No":
                ...
                msg.value = boss_warning("Incorrecto. Capacitación es gasto del periodo (no capitalizable).")

        b1.on_click(lambda _: check("Sí"))
        b2.on_click(lambda _: check("No"))

        show_screen(
            "Nivel 3: ¿Se capitaliza?",
            """
            La empresa pagó <b>$4.000</b> por <b>capacitación</b> del personal para operar la máquina.<br><br>
    ...
    if level == 4:
        ...
        def verify(_):
            ...
            try:
                ...
                if abs(val - 8000) < 1e-6:
                    ...
                    msg.value = boss_warning("Incorrecto. (88.000 − 8.000) / 10 = 8.000.")
            except:
                msg.value = boss_warning("Ingrese un número válido (solo números).")
        ...
        show_screen(
            "Nivel 4: Depreciación",
            """
            Datos del activo:<br>
            • Costo: 88.000<br>
            • Valor residual: 8.000<br>
            • Vida útil: 10 años<br><br>
            ¿Cuál es la <b>depreciación anual</b>?
            """,
            [inp, btn, msg],
            show_timer_note=True
        )
    ...
    if level == 5:
        ...
        def check(ans):
            ...
            if ans == "NIC 16":
                ...
                msg.value = boss_warning("Incorrecto. La obligación de depreciar PPE corresponde a NIC 16.")
        ...
        show_screen(
            "Nivel 5: Hallazgo crítico",
            """
            Detecta que la empresa <b>no está depreciando</b> la máquina.<br><br>
            ¿Qué norma está siendo violada?
            """,
            buttons + [msg],
            show_timer_note=True
        )
    ...
    if level == 6:
        ...
        show_screen(
            "✅ MISIÓN COMPLETADA",
            """
            Felicitaciones. Aplicó NIC 16 correctamente:<br>
            • Reconocimiento<br>
            • Medición inicial<br>
            • Costos no capitalizables<br>
            • Depreciación
            """,
            [btn_restart],
            show_timer_note=False
